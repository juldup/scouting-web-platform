...
class ListingPDF {
  ...
  protected function doDownloadListing($sections, $output, $exportPrivateData, $includeScouts, $includeLeaders, $groupBySection) {
    ...
    if ($this->output == 'excel') {
      ...
      $objWriter =  new Xlsx($excelDocument, 'Excel5');
      ...
      // Export to csv
      $objWriter = new \PhpOffice\PhpSpreadsheet\Writer\Csv($excelDocument);
...
*/      // Create pdf document
      ...
      foreach ($sections as $section) {
        ...
        $objWriter = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($excelDocument, 'Tcpdf');
        ...
        for ($i = 1; $i <= $pageCount; $i++) {
          ...
          $pdfDocument->AddPage('L', array($size['width'], $size['height']));
  ...
  protected function createExcelFile($delasection) {
    ...
    $excelDocument->getDefaultStyle()->getBorders()->getBottom()
            ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE);
    $excelDocument->getDefaultStyle()->getBorders()->getRight()
            ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE);
    $excelDocument->getDefaultStyle()->getBorders()->getLeft()
            ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE);
    $excelDocument->getDefaultStyle()->getAlignment()
            ->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
    $excelDocument->getDefaultStyle()->getFont()
            ->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
    */
    if (!$this->normalStyleArray) {
      $this->normalStyleArray =
        array('borders' => array(  
                      ...
                      'bottom'    => array('style' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE)
                    ),
            'alignment' => array(
  ...
  protected function fillInSheetForSection($excelDocument, $sheetIndex, $section, $csvMode = false, $includeScouts, $includeLeaders) {
    ...
    $hasSubgroup = $csvMode ? true :
      (Member::where('validated', '=', 1)
              ...
              ->where('subgroup', '!=', '')
              ->first() ? true : false);
    if (!$includeScouts) $hasSubgroup = false;
    ...
    if ($this->output == 'pdf') {
      ...
      if (!$hasTotem && !$hasSubgroup) {
        $colSizes = Array(4,24,19,7,12,39,5,21,16);
      } elseif ($hasSubgroup && !$hasTotem) {
        $colSizes = Array(4,21,17,7,19,12,29,5,21,16);
      } elseif ($hasSubgroup && $hasTotem) {
        $colSizes = Array(4,15,15,7,13,14,12,31,5,21,16);
      } elseif ($hasTotem && !$hasSubgroup) {
        $colSizes = Array(4,19,17,7,14,12,34,5,21,16);
      }
    } else {
      $colSizes = array();
    ...
    if ($this->output == "pdf") {
      ...
      $excelDocument->getActiveSheet()->getStyle("A$row")->applyFromArray($this->titleStyleArray);
    ...
    if (!$csvMode || $row == 1) {
      ...
      $excelDocument->getActiveSheet()->getStyle("A$row:$lastColumn$row")->applyFromArray($this->headerStyleArray);
      $row++;
    }
    // Get listing
    ...
    foreach ($members as $member) {
      if (!$nowShowingLeaders && $member->is_leader) {
        ...
        if ($this->output == 'pdf') {
          ...
          $excelDocument->getActiveSheet()->mergeCells("A$row:$lastColumn$row");
          $excelDocument->getActiveSheet()->getStyle("A$row")->applyFromArray($this->headerStyleArray);
          $row++;
        }
      }
      $letter = 'A';
      ...
      // Set style
      $excelDocument->getActiveSheet()->getStyle("A$row:$lastColumn$row")->applyFromArray($this->normalStyleArray);
      // Increment row
      $row++;
    }
    // Set column widths
    ...
    $excelDocument->getActiveSheet()->getPageSetup()
