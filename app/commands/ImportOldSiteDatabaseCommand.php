...
class ImportOldSiteDatabaseCommand extends \Illuminate\Console\Command {
  ...
  protected $sectionIdToIdArray = array();
  ...
  protected $rootFolder = "/home/julien/Websites/sv20-old";
  protected $newSiteRootURL = "http://www.sv20.be/";
  
  public function fire() {
    
    $pdo = new PDO('mysql:host=localhost;dbname=sv20_old', 'tmp', 'tmp', array());
    ...
    foreach ($query->fetchAll() as $section) {
      ...
      $this->sectionIdToIdArray[$section['sectionId']] = $sectionObject->id;
    ...
    foreach ($query->fetchAll() as $user) {
      try {
        $newUser = User::create(array(
            ...
            'email' => $user['email'],
            'default_section' => $user['ongletFixe'] ? $this->sectionIdToId($user['onglet']) : 1,
        ));
    ...
    foreach ($query->fetchAll() as $member) {
      ...
      $newMember = Member::create(array(
          ...
          'subscription_paid' => $member['paye'] ? true : false,
    ...
    foreach ($query->fetchAll() as $privilegeData) {
      if (!array_key_exists($privilegeData['listingId'], $this->members)) {
//        echo "Warning: skipping unknown member with id " . $privilegeData['listingId'] . "\n";
        continue;
      }
      ...
      elseif ($operationFR == "Supprimer des fiches santé #delasection" || $operationFR == "Supprimer des fiches santé") $privilege = null;
    ...
    foreach ($query->fetchAll() as $healthCard) {
      if (!array_key_exists($healthCard['idInListing'], $this->members)) {
//        echo "Warning: unknown user " . $healthCard['idInListing'] . " for health card\n";
        continue;
      }
    ...
    foreach ($query->fetchAll() as $photo) {
      ...
      if (!array_key_exists($oldAlbumId, $this->newAlbums)) {
        echo "Skip photo in nonexistant album $oldAlbumId\n";
        continue;
      }
      if ($this->newAlbums[$oldAlbumId] && $this->oldAlbums[$oldAlbumId]) {
        ...
        echo "Warning: old album $oldAlbumId does not exist.\n";
            ...
            var_dump($this->newAlbums);
            var_dump($this->oldAlbums);
            die($e);
          }
    }
  ...
  protected function sectionToId($sectionName) {
    ...
    if (array_key_exists($sectionName, $this->sections)) return $this->sections[$sectionName];
    if ($sectionName === "0") return 1;
    if (array_key_exists($sectionName, $this->sectionIdToIdArray)) return $this->sectionIdToIdArray[$sectionName];
    throw new Exception("Unknown section '$sectionName'");
  }
  
  protected function sectionIdToId($oldSectionId) {
    if ($oldSectionId === "0") return 1;
    return $this->sectionIdToIdArray[$oldSectionId];
  }
